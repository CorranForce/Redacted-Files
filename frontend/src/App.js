...
function App() {
  ...
  const handleDeclassify = useCallback(async () => {
    ...
    if (!selectedFile && !documentText.trim()) {
      toast.error("Upload a document or paste text");
      return;
    }
    ...
    try {
      ...
      const step_increment = 40 / active.length;
      ...
      for (let i = 0; i < active.length; i++) {
        ...
        setLoadingProgress(50 + step_increment * (i + 1));
      }
  ...
  const handleGenerateImage = useCallback(async (postId, postText, platform) => {
    ...
    try {
      ...
      setPosts((prev) =>
        prev.map((p) => (p.id === postId ? { ...p, image_base64: res.data.image_base64 } : p))
      );
  ...
  const canDeclassify =
    (selectedFile || documentText.trim()) && Object.values(platforms).some(Boolean);
  ...
  return (
    <div className="min-h-screen bg-[#09090b] relative">
      ...
      <Toaster
        ...
        toastOptions={{
          style: { background: "#18181b", border: "1px solid #3f3f46", color: "#f4f4f5", fontFamily: "JetBrains Mono, monospace", fontSize: "12px" },
        }}
      />

      <Header history={history} onLoadSession={handleLoadSession} />
        ...
        {step === "input" && (
          <div className="space-y-16">
            <div className="space-y-4 animate-fadeIn">
              <p
                className="font-mono text-xs uppercase tracking-[0.3em] text-[#22c55e]"
            ...
            <div className="animate-fadeIn-delay-3">
              <Button
                data-testid="declassify-btn"
                disabled={!canDeclassify}
                onClick={handleDeclassify}
                className="px-16 py-6 bg-[#f4f4f5] text-[#09090b] font-oswald text-base uppercase tracking-[0.2em] rounded-none border-2 border-transparent hover:bg-[#22c55e] hover:text-black disabled:opacity-20 disabled:cursor-not-allowed transition-colors duration-200 active:scale-[0.98]"
              >
                Declassify
              </Button>
            </div>
